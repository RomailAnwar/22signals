...
const projects = [
  { title: "MStore", description: "Mangoding is the biggest company...", bigImage: Bigimg1 },
  { title: "Beauty", description: "Mangoding is the biggest company...", bigImage: Bigimg2 },
  { title: "Jual", description: "Mangoding is the biggest company...", bigImage: Bigimg3 },
  { title: "MangStore", description: "Mangoding is the biggest company...", bigImage: Bigimg4 },
];
...
const PortfolioSection = () => {
  return (
    <div className="relative w-full bg-[#000202] text-white pt-20">
        ...
        <section className="container mx-auto px-4 md:px-8 relative z-10 py-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
            {projects.map((project, idx) => (
              ...
              >
                <div className="glass-card rounded-2xl relative overflow-hidden my-4 group h-[307px] w-full md:mr-4 p-4 md:p-6 grid grid-cols-1 md:grid-cols-[1fr_1fr] gap-4 md:gap-6 items-center">
                 <div className="flex flex-col justify-start">
                  ...
                  <div className="flex flex-col items-end justify-start space-y-2 md:space-y-4 relative">
                    ...
                    <img
                      ...
                      style={{  backgroundPosition: "right -5px top 50px", }}
                      alt={project.title}
                      className="w-full h-[150px] md:h-[180px] lg:h-[200px]  object-bottom object-cover rounded-lg transition-transform duration-300"
                    />
                  </div>
                </div>
              </div>
      ...
      >
        {/* Left & Right Overlays */}
        <img src={ShadowL} alt="Left Overlay" className="absolute left-0 top-0 h-full object-contain opacity-80 pointer-events-none" />
        <img src={Colons} alt="Right Overlay" className="absolute right-0 top-0 h-[586px] w-[823px] opacity-100 pointer-events-none" />
        <img src={ShadowR} alt="Right Shadow" className="absolute right-0 top-0 h-full object-contain opacity-100 pointer-events-none" />

        <section className="container mx-auto px-4 md:px-8 relative z-10 py-12">
          <h2 className="text-2xl md:text-3xl font-light text-blue-400 mb-8 md:mb-12">Proven Results 
            <img src={Union} alt="Union" className="ml-2 inline-block" />
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
            {testimonials.map((item, idx) => (
              <div
                key={idx}
                className="glass-card-2 bg-gray-900 p-4 rounded-xl text-gray-300 hover:bg-gray-800 transition flex flex-col gap-3 md:gap-4"
              >
                <div className="flex items-center justify-between w-full">
                  ...
                  <img src={Icon} alt={`${item.name}-extra`} className="w-10 h-8 object-cover" />
                </div>
                <p className="italic mt-2 text-sm md:text-base">{item.text}</p>
              </div>
            ))}
          </div>
        </section>
